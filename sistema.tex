...
cualesquiera tienen como intención \textit{survey($X_{i}$)} o cuando dos exploradores
tienen como intención \textit{probear($X_{i}$)} para el mismo \textit{i}, o cuando dos inspectores tienen como
intención \textit{inspect} estando en el mismo nodo. En cualquiera de estos
...
Recordando que los miembros del equipo se encuentran ordenados de acuerdo a la prioridad de sus intenciones, el sistema comienza calculando el puntaje según las posiciones \textbf{originales} de todos los agentes \textbf{excepto el que tiene la mayor prioridad para moverse}; dicho puntaje es tomado como \textit{puntaje base}, y a partir de éste se vuelve a correr el algoritmo de coloreo con la nueva posición hipotética (utilizando el predicado \texttt{h/1} mencionado en la sección de Base de conocimiento de la tesis) del segundo agente mejor priorizado; los dos puntajes se comparan, % mayor o igual? definir threshold?
...
Utilizando por última vez el ejemplo mencionado desde la introducción de la tesis, aplicando el sistema de resolución propuesto, sólo uno de los agentes realizaría la acción excluyente, permitiendo que el resto realice otro tipo de acciones que beneficien al equipo de manera diferente; en situaciones de conflicto más complejas, como una expansión descoordinada de una zona que resulte en la pérdida de la misma, el sistema impediría dicha pérdida, resultando además en que el equipo efectivamente expanda su zona de dominio, pero de una manera lo suficientemente cauta como para que los puntos obtenidos aumenten, y no se corra riesgo de pérdida de dominio.